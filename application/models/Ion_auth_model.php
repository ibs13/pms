...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @
	 **/
	public function register($email, $password, $additional_data = array(), $groups = array())
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, first_name, id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('id' => $id));
